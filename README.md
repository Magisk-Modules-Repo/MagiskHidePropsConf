...
- On Android Pie it is no longer possible to edit ro.build.fingerprint (and a bunch of other props). The SafetyNet fix from this module will not work until Magisk's resetprop tool has been updated (no ETA). The current Magisk Canary build (and other snapshot builds) have been updated to be able to edit all props on Android Pie.
